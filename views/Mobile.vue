  ...
  <!-- <div class="home"> -->
    ...
    <div class="d-flex flex-row">
      <v-carousel 
        ...
        height="250px">
        <v-carousel-item
          v-for="(gift,i) in giftycons"
          :key="i"
          cycle
          class="text-center"  >
          <img :src="gift.image_url" style="width:auto;height:200px;" @click="goGiftycon(i)" />
          </v-carousel-item>
      </v-carousel>
...
import ShowGiftycon from '@/views/ShowGiftycon.vue'
...
export default {
  name: 'Main',
  components: {
    HelloWorld, VueDaumMap, Dlgimg, ImageShow, Over1, Over2,ShowGiftycon
  },
  ...
  computed: { 
    ...
    //  }),
      ...
      dialogGifty() { 
        return this.$store.getters.getDlgGifty
      },
  ...
  methods: {
    ...
    goGiftycon(i) {
      let data = this.giftycons[i]
      this.$store.dispatch('changeGiftyData', {newMsg:data})
      this.$store.dispatch('changeDlgGifty', {newMsg:true})
      this.currentComponent = 'show-giftycon'
      console.log('Giftycon : ', i)
    },
...
<style >
  .v-icon.notranslate.fas.fa-circle::before {
        color: rgb(0, 255, 200);
    }
  .v-icon.notranslate.fas.fa-circle:active {
        color: rgb(255, 0, 170);
    }
  .car-act {
